...
export default function Transaksi() {
  const ref = useRef();
  const { kode_transaksi } = useParams();
  ...
  useEffect(() => {
    ...
    getPasien();
    getDokter();
    getListTreatmentSelected(kode_transaksi);
  ...
  const getKodeDanTime = async () => {
    let response = await axios.get(
      `/transaksi/kode_dan_time/${kode_transaksi}`
    );
  ...
  const openModalEditPerTreat = (id, nama, harga_satuan, qty, diskon) => {
    setErrors([]);

    setIdPerTreatment(id);
    setNamaPerTreatment(nama);
    setHargaPerTreatment(harga_satuan);
  ...
  const masukanKeKeranjang = async (id, nama, harga, kode) => {
  ...
  const hapusPerTreatment = async (id, nama, kode) => {
    try {
      ...
      if (response.data.reload === true) {
        navigate("/transaksi");
        var tran = document.getElementById("transaksi");
        tran.click();
      } else {
        getListTreatmentSelected(kode);
      }
  ...
  const transaksiLagi = () => {
    ...
    toast.success("Transaksi pending selesai", {
      position: "top-center",
      duration: 2000,
      iconTheme: {
        primary: "#1bff1f",
